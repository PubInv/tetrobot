...
The Tetrobot Project seeks to build Tetrobot, a robotic material that moves like a slug but is strong like a truss. We are exploring using networks of linear actuators connected in triangular shapes to build strong space frames that can ooze into position. This a version of the TETROBOT concept from 20 years ago made accessible to hobbyist.

The Tetrobot Project is a part of <a href="https://github.com/PubInv/PIFAH">Public Invention</a>, which seeks to invent things directly beneficial to all humanity without seeking profit or patents. The project was started in August, 2015.
...
Currently the Tetrobot Project is Robert L. Read, full time, and Joshua Hannan and a number of students working as part time volunteers, although we welcome additional assistance.
We have acheived a crawling robot made of three tetrahedra. Technically, this a "5TetTetrobot", but we are seeking a more personal name.
<iframe width="560" height="315" src="https://www.youtube.com/embed/zl0AEfxyVMw" frameborder="0" allowfullscreen></iframe>

You may wish to subscribe to our <a href="https://www.youtube.com/watch?v=1T8XSMhwKUE">YouTube Channel</a>, since because motion is so intrinsic to the Tetrobot Project, videos are good way to show what we are doing.

The Tetrobot uses commercial linear acuators and the "Turret Joint", an invention of Kwon, Song and Kim, implemented by Rob using a 3D printer. It is controlled by two Arduino Mega with an open-source shield we designed ourselves to allow up to 6 motors to be controlled with a single Mega. An Emacs lisp program communicates via bluetooth with the two controller, giving us a command line interface to control the robot.
...
<figure>
	<img width="80%" src="https://github.com/PubInv/gluss/raw/gh-pages/images/robotTruckChasm.png"  alt="TetrobotBot Spannig Chasm, by WantedHero on Fiverr"/>
  <figcaption>TetrobotBot Spanning Chasm, by <a href="https://www.fiverr.com/wantedhero">WantedHero</a> on Fiverr.</figcaption>
</figure>
...
<ul>
<li> We need someone to sketch drawings of the "moving bridge" concept and other uses of Tetrobot to fire the imagine of would-be participants.
</li>
...
<li> We need several instructables produced, about the Turret Joint, the Tetrobot, and the Mega shield.
</li>
</ul>
...
## Subprojects Spawned from The Tetrobot Project
...
design of a continuum of tetrahelices. In particular, this allows us to "untwist" the Boerdijk-Coxeter helix, turning our actual 7TetTetrobot robot into a hexapod
robot.
</li>
<li>
...
## The Intellectual Underpinnings of The Tetrobot Project
...
The Tetrobot Projects engenders many research opportunities.
	  
### Build a Simulator

We need a software-only simulator so that gaits and other forms of motion can be investigated and developed without having to actually have Tetrobot. Ideally this would have a 3D rendering display and allow both programatic and interactive design of motion.

Note: The simulator is in an early, poorly documented and buggy stage, but is available.

### Build a physical model with sensors

It would be magical to control a Tetrobot by holding in your hands a small model of the robot that you can move and reshape. Such a model would not need acutators, but each member would need to sense and report its length. Then a change with your hands would immediately create a change in the Tetrobot, allowing you to control it elegantly.
...
### Explore more numerous tetrahedral Tetrobot systems

Our immediate plans are to build a 5-tetrahedron Tetrobot and explore if it can crawl faster than the 3-Tet system. However, we could build and control significantly larger systems.

Note: We currently have built a 7-tetradra Tetrobot system.

### Build a very small Tetrobot

It is interesting to investigate what is the smallest Tetrobot system we can construct.

### Build a Tetrobot out of Hydraulic Actuators
...
### Research Mathematics of Tetrobot Dancing
