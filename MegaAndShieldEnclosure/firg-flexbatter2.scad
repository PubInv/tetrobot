...
module arc(r1,r2,h,a1=0,a2=0){
    f = (-r1-r2);
     if(a2-a1<=180){
	   ...
	   rotate(a2)translate([f,0,-1])cube([2*(r1+r2),2*(r1+r2),h+2]);
	   rotate(a1+180)translate([f,0,-1])cube([2*(r1+r2),2*(r1+r2),h+2]);
        }
     } else {
           difference(){
              cylinder(r=r2,h=h);
...
module sline(angle,radius,i,w,h){
   scale([angle[i]>=0?1:-1,1,1])
      assign(r=abs(radius[i]))assign(a=angle[i])
         translate([a?r:0,0,0]){
	    translate([-w/2,-r,0])cube([w,0,h]); // tiny overlap!
            if(a)scale(1.01)arc(r-w/2,r+w/2,0,a,h=h);
                // Note the 1% scaling is a crummy fudget to make the model manifold....
	    else if(r>0)translate([-w/2,-r,0])scale(1.01)cube([w,r,h]);
      if(i+1<len(angle))
           rotate(angle[i])
...
module test_sline(n=1,l=65,d=18,hf=0.75,r=4,shd=3,eps=0.28,el=0,xchan=[1/4,3/4],$fn=24) {
...
deepen=0; //deepening for side grip of batteries
  ...
  //            for(sy=[-1,1])scale([1,sy,1])assign(D=d+2*w-2*ws-0.7){
		  ...
		  //sline([90,0,120,90,120,90,0],[d/8+2,d/6,d/8-1,d/8,-d/8,d/8,d/2],0,ws,hf*d+w);
                // I am getting non-manifold edges here, possibly just need to extend segment length? Need to understand how Sline works.   
		  sline([0,180],[r+ch+el,D/4],0,ws,hf*d+w);
  //          }
}
...
r = d/5+2*ws; // linear spring length (depends on sline() call!)
   ...
   for(i=[0:n-1])translate([0,i*(d+w+ws),0]){ // generate n battery cases
       // Ugly -- try scaling by 1% to get rid of non-manifold problem --- this requires more consideration.
       scale(1.01)
      difference(){
...
module flexbatterWFirg18650(n=43) {
//    flexbatter18650(n);
    flexbatter18650(n=3);
    translate([65*0.5,0,-(mount_length+mount_wall*2)/2])
      firg_mount();
    translate([65*0.5,40,-(mount_length+mount_wall*2)/2])
      firg_mount();
}
