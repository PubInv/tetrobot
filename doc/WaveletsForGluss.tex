...
\section{On interesting Dyanmics}

This section probably has nothing to do with dynamics, it is just an interesting idea.

Suppose that we have a gluss system, in the form of tentacle.  Let's imagine it is just in two
dimensions for now.

How fast can we launch a payload? How fast can we throw something?

Note that we can think of this problem in terms of kinematics (which is kind of a cheap shot,
and unlikely to be successful) just based on the motion of the actuators, assuming they are always
strong enough to produce the velocities in question.

Or we can work on it from the point of view of dynamics, which makes it much more interesting.

A gluss tentacle that could toss things would be pretty astounding.


\section{Thoughts on Wavelets}

Over the week will traveling I made a number of attempts to develop a theory of
Blob Spaces.

There are several approaches:
\begin{itemize}  
\item We can create a ``moment-based'' system.
\item We can try to build a ``simplex'' based system that uses triangles/tetrahedra.
\item We can beuild an ``extent-based'' system.
\end{itemize}

I made copious notes with many ideas but came to no particular conclusion.
However, I am convinced that we need a software toolchain and that the basic ideas of
Blob theory are needed.

Note that I need to do a lot more research in animation to understand this.

My notes suggest that there are rich possibilities, but I found no crisp
theorems or ideas.

One idea is to separate the physical interpretation from the data space.

Resarch ``Fresh'' and 2D animation.
3D Animation seems to be done with Blender or ``Maya'' and rigging and keyframes.
Motion Graphics is next form.
